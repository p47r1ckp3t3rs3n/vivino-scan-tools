...
# --- Constants ---
API_BASE = "https://api.testing.vivino.com/vintages/"
CACHE_FILE = "vintage_metadata_cache.json"
GITHUB_BASE = "https://raw.githubusercontent.com/p47r1ckp3t3rs3n/vivino-test-images/main/images/"

# --- Fetch metadata from API ---
def fetch_vintage_metadata(vintage_id):
    try:
        url = f"{API_BASE}{int(vintage_id)}"
        print(f"🔄 Fetching metadata for vintage ID {vintage_id}...")
        ...
        print(f"⚠️ Failed to fetch metadata for {vintage_id}: {e}")
...
# --- Enrich dataset ---
def enrich_metadata(df, label, cache):
    ...
    for idx, row in df.iterrows():
        ...
        if vintage_id not in cache:
            cache[vintage_id] = fetch_vintage_metadata(vintage_id)
...
def compare_results(fileA, fileB, output_xlsx, use_cache=False):
    ...
    df = enrich_metadata(df, labelA, cache)
    df = enrich_metadata(df, labelB, cache)
    if use_cache:
    ...
    df = df[side_by_side_order + other_cols]
...
if __name__ == "__main__":
    ...
    compare_results(args.fileA, args.fileB, args.output, args.use_cache)